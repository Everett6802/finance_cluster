...
unsigned short ClusterMgr::set(ParamType param_type, void* param1, void* param2)
{
    ...
    switch(param_type)
    {
		case PARAM_FILE_TRANSFER_TOKEN_REQUEST:
		{
			...
			{
				if(pthread_mutex_trylock(&file_tx_mtx))
				{
					file_tx_token = gen_random_string();
					if (param1 != NULL)
						*(string*)param1 = gen_random_string();	
					WRITE_DEBUG("Request file transfer token");				
				}
				else
    	...
    	case PARAM_FILE_TRANSFER:
    	{
			if (!file_tx_token.empty())
    		{
    			WRITE_ERROR("The file transfer resource is busy. Incorrect operation !!!");
    			return RET_FAILURE_INCORRECT_OPERATION;
    		}
        	if (param1 == NULL || param2 == NULL)
    		{
