...
unsigned short EventFileAccess::read(list<PEVENT_ENTRY>* event_list, list<string>* event_line_list)
{
	unsigned short ret = RET_SUCCESS;
	...
	while(iter != line_list->end())
	{
		...
		while ((line_field_tmp = strtok_r(line, "|", &rest_line_field)) != NULL)
		{
			string line_field;
...
unsigned short EventRecorder::read(list<EventEntry*>* event_list, list<string>* event_line_list)
{
	unsigned short ret = RET_SUCCESS;
	assert(event_list != NULL && "event_list should NOT be NULL");
    ret = event_device_access->read(event_list, event_line_list);
	return ret;
}